...
describe('App series symbology', () => {
  ...
  it('renders pre-2020 KPHD fecal observations as light green points in the E. coli series', () => {
    ...
    expect(src).toContain('Light green observations in the &quot;E. coli observations&quot; series are actually KPHD Fecal Coliform freshwater observations from before 2020.')
  })

  it('colors E. coli GMV30 segments light green while their rolling windows include fecal coliform values', () => {
    const fs = require('fs')
    const path = require('path')
    const srcPath = path.resolve(__dirname, 'App.jsx')
    const src = fs.readFileSync(srcPath, 'utf8')
    expect(src).toContain("GMV30IncludesFecal: false")
    expect(src).toContain("out[index].GMV30IncludesFecal = windowRows.some((row) => row.Constituent === 'Fecal Coliform')")
    expect(src).toContain("const ecoliGmvSegments = splitGmvSegments(ecoliGmvSeries, 'EcoliGMV', 'EcoliGMVIncludesFecal')")
    expect(src).toContain('stroke={segment.includesFecal ? KPH_D_FECAL_OBSERVATION_COLOR : KPH_D_ECOLI_OBSERVATION_COLOR}')
    expect(src).toContain('showKphdFecalGmvNote: ecoliGmvSegments.some((segment) => segment.includesFecal)')
    expect(src).toContain('Light green portions of the E. coli GMV30 line use rolling 30-observation-day windows that include KPHD Fecal Coliform freshwater observations from before 2020.')
  })
  ...
  it('uses unique chart data keys and two-decimal tooltip formatting', () => {
    ...
    expect(src).toContain('const ecoliGmvSeries = sampleEcoliStats')
    expect(src).toContain('const ecoliGmvSegments = splitGmvSegments')
    ...
    expect(src).toContain('data={segment.points}')
